    ...
    try {
      ...
      rec.setOnRecordingStatusUpdate(async (st: any) => {
        ...
        if (utteranceTooLong || silenceLongEnough) {
          ...
          const shouldDiscardUtterance =
            discardCurrentUtteranceRef.current && !allowBargeInRef.current;
          discardCurrentUtteranceRef.current = false;

          if (shouldDiscardUtterance) {
  ...
  const startListening = useCallback(async () => {
    ...
    try {
      ...
      const processRecordedChunks = async (recordedChunks: Int16Array[]) => {
        if (!activeRef.current) return;
        if (discardCurrentUtteranceRef.current && !allowBargeInRef.current) {
          discardCurrentUtteranceRef.current = false;
          onSpeechEndRef.current?.();
          return;
        }
        ...
        onSpeechStart: () => {
          ...
          if (isSpeakingRef.current) {
            if (allowBargeInRef.current) {
              ...
              console.log("Ignoring speech while AI is speaking");
              discardCurrentUtteranceRef.current = true;
            }
          }
        },
